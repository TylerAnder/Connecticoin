...
const char * const BITCOIN_CONF_FILENAME = "connecticoin.conf";
...
#else
    const char* pszModule = "connecticoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ConnectiCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ConnectiCoin";
#else
    // Unix
    return pathRet / ".connecticoin";
#endif
#endif
}
